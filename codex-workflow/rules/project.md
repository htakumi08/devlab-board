...
- 要約には、どの機能を実装・変更したコミットなのか簡潔に分かる文言を記述する。
- `更新`、`修正`、`対応`だけの曖昧なメッセージを避け、対象と変更内容を含める。
- 一つのコミットには一つの目的を持たせ、メッセージと実際の差分を一致させる。
- コミット前にstage済み差分を確認し、例えば `機能: Finance口座一覧・詳細APIを追加`、`文書: Finance口座機能の設計書を追加` のように表す。
