...
- 1行目は `<種別>: <要約>` の形式とし、先頭だけでコミットの種類を判断できるようにする。
- 種別は `機能`、`修正`、`文書`、`テスト`、`リファクタ`、`保守`、`インフラ`、`運用` から、差分の主目的に合うものを選ぶ。
- 要約は、実装・変更した機能が簡潔に分かる具体的な文言にする。
- `更新`、`修正`、`対応`だけの曖昧な表現を避け、対象と変更内容を含める。
- 一つのコミットには一つの目的を持たせ、メッセージと差分を一致させる。
- 例: `機能: Finance口座一覧・詳細APIを追加`、`修正: セッション切れ後に口座情報が残る問題を解消`、`文書: Finance口座機能の設計書を追加`。
...
| 2026-08-11 | コミットメッセージを日本語の `<種別>: <要約>` 形式とする規則を追加 |
